...
require_once('sanitize.php');
	...
	//Create query
	$qry="SELECT * FROM members WHERE login='$login' AND passwd='".md5($_POST['password'])."' AND `status`='active'";
	...
	if($result) {
		if(mysql_num_rows($result) == 1) {
			...
			$_SESSION['SESS_PENDREQ'] = $member['pending_requests'];
			$_SESSION['SESS_STATUS'] = $member['status'];
			$_SESSION['ACTIVATION_CODE'] = $member['activation_code'];
